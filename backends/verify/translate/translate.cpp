...
Translator::Translator(std::ostream &out, P4VerifyOptions &options, BMV2CmdsAnalyzer* bMV2CmdsAnalyzer) 
    ...
    // init oldProcedure
    oldProcedure = BoogieProcedure("oldProcedure");
    oldProcedure.addDeclaration("procedure {:inline 1} oldProcedure()\n");
    oldProcedure.setImplemented();
    ...
    // Assert feature
    declaration += "var " + assertDummyName + ":bool;\n";
    addGlobalVariables(assertDummyName);
...
void Translator::translate(const IR::P4Action *p4Action){
    ...
    // move to translate table, it's wrong in action translation
